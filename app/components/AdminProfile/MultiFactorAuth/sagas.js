...
import {
  verifyMultiFactorAuthSuccess,
  verifyMultiFactorAuthFailure,
  getMultiFactorAuthSuccess,
  getMultiFactorAuthFailure,
  disableMultiFactorAuthSuccess,
  disableMultiFactorAuthFailure,
  getRecoveryCodesSuccess,
  getRecoveryCodesFailure,
  sendMultiFactorRecoveryCodesEmailSuccess,
  sendMultiFactorRecoveryCodesEmailFailure,
  generateRecoveryCodeSuccess,
  generateRecoveryCodeFailure,
  loadBasicInfoSuccess,
  loadBasicInfoFailure,
  enable2faAuthSuccess,
  enable2faAuthFailure,
} from './actions';
...
//   yield fork(
//     API.post(
//       `multi-factor-auth/totp-verify?secret=${action.secret}`,
//       verifyMultiFactorAuthSuccess,
//       verifyMultiFactorAuthFailure,
//       action.payload,
//       token,
...
//   yield fork(
//     API.put(
//       `multi-factor-auth/totp-disable/${action.payload}`,
//       disableMultiFactorAuthSuccess,
//       disableMultiFactorAuthFailure,
//       { _id: action.payload },
//       token,
...
function* disableMultiFactorAuthFlow(action) {
  ...
  yield fork(
    API.post(
      ...
      disableMultiFactorAuthSuccess,
      disableMultiFactorAuthFailure,
      { email: userEmail, otp: totp_token },
      token,
    ),
  );
  yield take([LOCATION_CHANGE, types.DISABLE_MULTI_FACTOR_AUTH_FAILURE]);
...
function* getRecoveryCodesFlow() {
  ...
  yield fork(
    API.get(
      'multi-factor-auth/recovery-code/get',
      getRecoveryCodesSuccess,
      getRecoveryCodesFailure,
      token,
    ),
...
function* sendEmailRecoveryCodesFlow(payload) {
  ...
  yield call(
    API.get(
      `multi-factor-auth/recovery-code/send/${payload.userId}`,
      sendMultiFactorRecoveryCodesEmailSuccess,
      sendMultiFactorRecoveryCodesEmailFailure,
      token,
    ),
...
function* generateRecoveryCodesFlow(payload) {
  ...
  yield call(
    API.put(
      `multi-factor-auth/generate/recovery-code/${payload.userId}`,
      generateRecoveryCodeSuccess,
      generateRecoveryCodeFailure,
      { _id: payload.userId },
      token,
